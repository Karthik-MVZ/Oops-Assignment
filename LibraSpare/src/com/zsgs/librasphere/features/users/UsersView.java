...
import com.zsgs.librasphere.features.users.manage.ManageView;
import com.zsgs.librasphere.features.users.search.SearchView;
import com.zsgs.librasphere.features.users.details.DetailsView;
...
public class UsersView extends BaseView {
    ...
    public void displayUsers(java.util.List<com.zsgs.librasphere.repository.dto.User> users) {
        if (users.isEmpty()) {
            System.out.println("\nNo users found.");
            return;
        }
        ...
        for (com.zsgs.librasphere.repository.dto.User user : users) {
            System.out.printf("%-15s %-20s %-20s %-15s %-15s%n",
                    user.getUserId(),
                    user.getFirstName(),
                    user.getLastName(),
